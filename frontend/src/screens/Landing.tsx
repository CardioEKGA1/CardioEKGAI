  ...
  <div style={{minHeight:'100vh',display:'flex',flexDirection:'column'}}>
    ...
    <nav style={{padding:'16px 40px',display:'flex',alignItems:'center',justifyContent:'space-between',background:'rgba(255,255,255,0.6)',backdropFilter:'blur(10px)',borderBottom:'1px solid rgba(122,176,240,0.2)'}}>
      <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
        ...
        <div>
          <div style={{fontSize:'16px',fontWeight:'800',color:'#1a2a4a'}}>CardioEKGAI</div>
          <div style={{fontSize:'9px',color:'#8aa0c0',letterSpacing:'1px',textTransform:'uppercase'}}>Clinical Decision Support</div>
        </div>
      </div>
      <div style={{display:'flex',gap:'10px'}}>
        ...
        <button onClick={onSignUp} style={{background:'linear-gradient(135deg,#7ab0f0,#9b8fe8)',border:'none',borderRadius:'10px',padding:'8px 20px',fontSize:'13px',fontWeight:'700',color:'white',cursor:'pointer'}}>Get Started Free</button>
      </div>
    </nav>
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'60px 40px',textAlign:'center'}}>
      ...
      <div style={{display:'flex',gap:'14px',marginBottom:'60px',flexWrap:'wrap',justifyContent:'center'}}>
        <button onClick={onSignUp} style={{background:'linear-gradient(135deg,#7ab0f0,#9b8fe8)',border:'none',borderRadius:'16px',padding:'16px 36px',fontSize:'16px',fontWeight:'700',color:'white',cursor:'pointer'}}>Start Free Analysis</button>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'20px',maxWidth:'800px',width:'100%'}}>
        {[
          ...
          {icon:'💬',title:'AI Cardiology Chat',body:'Ask Dr. CardioEKGAI follow-up questions about the findings with full context'},
